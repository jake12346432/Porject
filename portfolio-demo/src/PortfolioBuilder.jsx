...
import { getAIPortfolioConfig } from "./api.js";
...
// Best-effort mapping to Investopedia's glossary — Investopedia blocks
// automated verification, so these were picked from general knowledge, not
// confirmed live. Value/Growth/Momentum/Dividend are long-standing, stable
// glossary terms and very likely correct; Quality and ESG are the least
// certain (Investopedia doesn't have as clean a single canonical page for
// "quality investing" specifically, so it points at the broader "Factor
// Investing" entry instead). Click through each once live and report back
// anything that lands wrong — this table is the only thing that needs fixing.
const INVESTOPEDIA_LINKS = {
  Quality: "https://www.investopedia.com/terms/f/factor-investing.asp",
  Value: "https://www.investopedia.com/terms/v/valueinvesting.asp",
  Growth: "https://www.investopedia.com/terms/g/growthinvesting.asp",
  Momentum: "https://www.investopedia.com/terms/m/momentum-investing.asp",
...
function ConceptLink({ term, children, style }) {
  const href = INVESTOPEDIA_LINKS[term];
  if (!href) return <>{children}</>;
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" title={`${term} investing, explained on Investopedia`}
      style={{ color: "inherit", textDecoration: "none", borderBottom: "1px dotted currentColor", cursor: "pointer", ...style }}>
      {children}
    </a>
  );
}
...
/* ============================== MAIN ============================== */
// `rpq` (the target split from the risk questionnaire, {equityPct, fiPct}) and `onBuyComplete`
// (called once a buy is saved, with everything the flow orchestrator needs to move on to the
...
// rendered now.
export default function PortfolioBuilder({ theme, setTheme, rpq, onBuyComplete, onBack, onStepClick, reachableSteps }) {
  ...
  // Target order value for this leg — pricing and saving happens later, together with the Fixed
  // Income leg, at the Portfolio Summary step (see onBuyComplete below).
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: t.bg, color: t.text, minHeight: "100vh", fontSize: 14, transition: "background 0.25s, color 0.25s" }}>
      ...
      <div style={{
        ...
        <FlowBreadcrumb t={t} step="equity" onStepClick={onStepClick} reachable={reachableSteps} />
      ...
      <div style={{ maxWidth: 1160, margin: "0 auto", padding: "40px 28px 80px" }}>
        ...
        {rpq && (
          <div style={{
            ...
            Your target allocation: <strong style={{ color: t.textStrong }}>{rpq.equityPct}% Equity</strong> / {rpq.fiPct}% Fixed Income — build this portion first, then Fixed Income.
          </div>
        )}
        ...
        <div style={{ marginBottom: 20 }}>
          <FilterCard t={t} wide heading="What matters most to you" description="Rate how much each of these four things should count when we score a stock. Raising one lowers the others — they always add up to 100%.">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 20 }}>
              ...
              <div>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: t.positive, marginBottom: 4 }}><ConceptLink term="Value">Value</ConceptLink></div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(420px, 1fr))", gap: 20, marginBottom: 36 }}>
          <FilterCard t={t} heading={<><ConceptLink term="Dividend">Dividend</ConceptLink> income</>} description="Companies that regularly pay shareholders cash — useful for income, not just growth.">
          ...
          <FilterCard t={t} heading={<>Responsible investing (<ConceptLink term="ESG">ESG</ConceptLink>)</>} description="A score for environmental impact, employee treatment, and governance quality.">
        ...
        {portfolio && stats && (
          <div ref={resultsRef}>
            ...
            {/* REVIEW & CONTINUE — no buy happens here anymore: this portfolio is priced, sized,
                and saved together with the Fixed Income portion in one combined order at the
            ...
            <div style={{ marginTop: 26 }}>
              ...
              <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 10, padding: 16 }}>
                <div style={{ fontSize: 12, color: t.muted, lineHeight: 1.6, marginBottom: 12 }}>
                  This is priced and saved together with your Fixed Income portion, as one combined
