...
export default function PortfolioBuilder() {
  ...
  // Lock & buy — order ticket generation, not real trade execution
  ...
  const runAI = async () => {
    ...
    try {
      const systemPrompt = `You convert a natural-language portfolio request into a JSON configuration for a stock screener. Respond with ONLY a raw JSON object — no markdown fences, no prose before or after.
...
Infer sensible values. Leave things at neutral defaults if not mentioned (equal QVGM, no sector/location tilt, all regions/sectors included, thresholds disabled). The portfolio always shows the top 10 highest-scoring stocks — there is no holding-count setting.`;
      ...
      setAiError("Couldn't turn that into a portfolio config — try rephrasing, or adjust the filters manually below.");
    } finally {
      setAiLoading(false);
    }
  };
...
${list}`;
  ...
  // "Lock & buy" — builds a sized order ticket from live-priced holdings. This does not place any
  // real trade: there is no brokerage connection here, only a downloadable ticket for the person
  // to execute themselves. Not financial advice.
  const lockPortfolio = () => {
    const withPrice = selected.filter(s => liveData[stockKey(s)] && liveData[stockKey(s)].price);
    if (withPrice.length === 0) return;
    const wSum = withPrice.reduce((a, s) => a + s.weight, 0) || 1;
    const holdings = withPrice.map(s => {
      const w = s.weight / wSum; // renormalized among just the live-priced subset
      const price = liveData[stockKey(s)].price;
      const amount = w * portfolioValue;
      return { ticker: s.ticker, name: s.name, sector: s.sector, country: s.country, weight: w * 100, price, shares: amount / price, amount };
    });
    const totalAllocated = holdings.reduce((a, h) => a + h.amount, 0);
    setLockedPortfolio({ timestamp: new Date().toLocaleString(), portfolioValue, holdings, totalAllocated, cash: portfolioValue - totalAllocated });
  };
                ...
                {Object.keys(liveData).length > 0 && (
                  <>
                    ...
                    <div style={{ marginTop: 16, display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                      ...
                      <button className="tw-btn-primary" onClick={lockPortfolio} style={{
                        padding: "10px 20px", borderRadius: 99, border: "none", background: `linear-gradient(135deg, ${t.positive}, #1a8f5c)`, color: "#FFFFFF",
                        fontWeight: 600, fontSize: 12, letterSpacing: "0.06em", textTransform: "uppercase", cursor: "pointer",
                      }}>🔒 Lock & generate order ticket</button>
                    </div>
                    <div style={{ fontSize: 10.5, color: t.faint, marginTop: 6 }}>
                      Sizes an order ticket using only the holdings with live pricing above (weights renormalized among just
                      those names). This does not place any real trade — Claude has no brokerage connection. Not financial advice.
                    </div>
                  </>
                )}
              </div>
              ...
              {lockedPortfolio && (
                <div style={{ marginTop: 16, background: t.surface, border: `1px solid ${t.lockBorder}`, borderRadius: 10, padding: 16 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12, flexWrap: "wrap", gap: 10 }}>
                    <div>
                      <div style={{ fontSize: 13, color: t.positive, fontWeight: 600 }}>🔒 Locked order ticket</div>
